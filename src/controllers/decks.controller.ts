...
// Establecer carta de portada del deck
export const setDeckCoverCard = async (req: Request, res: Response) => {
  try {
    const user = (req as any).user;
    const { id } = req.params;
    const { card_id } = req.body; // null para limpiar portada

    const deck = await DeckService.setDeckCoverCard(user.id, id, card_id ?? null);
    return res.json(deck);
  } catch (error: any) {
    console.error('Error estableciendo portada del deck:', error);
    if (error instanceof ServiceError) {
      return res.status(error.status).json({ error: error.message });
    }
    return res.status(500).json({ error: 'Error estableciendo portada del deck' });
  }
};
