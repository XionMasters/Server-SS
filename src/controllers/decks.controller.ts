...
export const createDeck = async (req: Request, res: Response) => {
  try {
    ...
    // Obtener el dorso por defecto para asignarlo automáticamente
    ...
    const deck = await Deck.create({
      ...
      is_active: false,
      current_deck_back_id: defaultDeckBackId  // Asignar dorso por defecto
    });
...
export const autoGenerateDeck = async (req: Request, res: Response) => {
  try {
    ...
    // Pasar tanto IDs como cantidades al generador
    ...
    const generatedDeck = await generateAndSaveDeck(userId, id, userCardData, options);
